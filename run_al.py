...
def max_variance(tokenizer, estimator, data_dir, model_dir, pool_name,
                 n_instances, sample_size, label_distn, max_seq_length, use_tpu,
                 batch_size, overwrite_tfrecord):
    del label_distn  # Unused.
    pool_df = pd.read_csv(f"{data_dir}/{pool_name}.tsv", sep="\t")
    n_instances_per_label = int(n_instances / len(multilabel.LABEL_COLUMN))
...
def al_teach(task_name,
    ...
    model_fn = multilabel.model_fn_builder(
        ...
        label_mask=[1] * len(multilabel.LABEL_COLUMN))
...
def active_learning_procedure(query_strategy, tokenizer, bert_config, data_dir,
                              ...
                              init_task_name, n_queries, n_instances,
                              sample_size, num_init_train_epochs,
                              num_query_train_epochs, retrain_all,
                              overwrite_tfrecord):
    query_func = uniform
    if query_strategy == "max_variance":
        query_func = max_variance
    elif query_strategy == "max_variance_balanced":
    ...
    estimator, result = al_teach(f"{init_task_name}_al", tokenizer, bert_config,
                                 f"{output_dir}/init", data_dir,
                                 num_init_train_epochs, tpu_cluster_resolver,
                                 is_per_host)
    perf_history = [result]

    init_train_df = pd.read_csv(f"{data_dir}/{init_task_name}_al.tsv", sep="\t")
    label_distn = np.array(
        [sum(init_train_df[l] == 1) for l in multilabel.LABEL_COLUMN])
    label_distn = label_distn / sum(label_distn)
    for i in range(n_queries):
        if i == 0:
            model_dir = f"{output_dir}/init"
            model_data_dir = data_dir
            pool_name = f"{init_task_name}_pool"
            if retrain_all:
                train_df = init_train_df
        else:
            model_dir = f"{output_dir}/{query_strategy}-{i}"
            model_data_dir = f"{data_dir}/{finetune_module}-{query_strategy}"
